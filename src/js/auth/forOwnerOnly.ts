 ...
 */
const forOwnerOnly = <P extends ForOwnerOnlyProps = ForOwnerOnlyProps>(Component: Fn<P>): (props: P) => React.FunctionComponentElement<P> | null => {
  if (Component == null) return () => null
  const AuthenticatedFC = (props: P): null | React.FunctionComponentElement<P> => {
    const { isAuthorized } = usePermissions({ currentUserUuid: props.userUuid })
    if (isAuthorized) {
