...
function VerifyEmailPendingPage (): JSX.Element {
  ...
  const { error } = useSWR<TokenHelperType>(
    token != null ? url : null,
    fetcher,
    { refreshInterval: 0 }
  )
  ...
  return (
    <>
      <SeoTags title='Email verification pending - OpenBeta' />
        ...
        <section className='bg-base-100 max-w-sm mx-auto px-4 py-12 rounded-box'>
          <div className='flex flex-col gap-4 items-center'>
            ...
            <div className='mt-4 flex flex-col gap-4 items-center w-full'>
              ...
              <button
                className='btn btn-ghost btn-sm text-sm text-base-300'
                onClick={() => {
                  void signIn('auth0')
                }}
              >
                Already verified? Login{' '}
                <ArrowSmallRightIcon className='ml-2 w-5 h-5' />
              </button>
            </div>
          </div>
        </section>
