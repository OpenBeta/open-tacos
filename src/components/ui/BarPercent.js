      ...
      <div className={`h-2 w-100 flex ${styles}`}>
        {
          percents.map((percent, index) => {
            ...
            if (moreThanOnePercent && index === 0) { borderRounding = 'rounded-l mr-1' }
            if (moreThanOnePercent && index === percents.length - 1) { borderRounding = 'rounded-r' }
            if (!moreThanOnePercent) { borderRounding = 'rounded-lg' }
