...
const GitHubStars: React.FC = () => {
  ...
  return (
    <a className='no-animation transition-none flex items-center border rounded-btn gap-1 px-2' href='https://github.com/OpenBeta/open-tacos'>
      <GithubIcon className='w-5 h-5' /> Star  <span className='px-2 py-1 backdrop-brightness-75 rounded text-xs'>{data?.stargazers_count}</span>
    </a>
  )
}
