...
const ClimbSearch = ({ expanded, onClick, onClickOutside }: ClimbSearchProps): JSX.Element => {
  ...
  return (
    <div className='fixed top-0 left-0 w-screen horizontal-center pointer-events-none'>
      <div id='searchPanel' className='max-w-screen-lg horizontal-center w-full flex flex-col items-center'>
        <FakeSearchBox onClick={onClick} expanded={expanded} />
        {expanded && <div className='py-4 text-secondary-contrast pointer-events-auto'>Find climbs by name, style or FA</div>}
...
export const FakeSearchBox = ({ onClick, expanded }: {onClick: any, expanded: boolean}): JSX.Element => {
  return (
    <div className='py-2 pointer-events-auto' onClick={onClick}>
      <div className={`${expanded ? 'hidden h-0' : 'block'} cursor-pointer mt-0.5 border border-gray-200 shadow-lg shadow-inner rounded-full flex flex-row items-center gap-x-4 py-0.5 bg-white`}>
