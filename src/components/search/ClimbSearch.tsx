...
const ClimbSearch = ({ expanded, onClick, onBlur }: ClimbSearchProps): JSX.Element => {
  ...
  return (
    <div className='fixed top-0 left-0 w-screen horizontal-center z-10'>
      <div className='max-w-screen-lg horizontal-center w-full flex flex-col items-center '>
        ...
        {expanded && <div className='py-4 text-secondary-contrast'>Find climbs by name, characteristics or FA</div>}
        <div className={`${expanded ? 'w-full horizontal-center ' : 'hidden'}`}>
          <Autocomplete
...
export const FakeSearchBox = ({ onClick, expanded }: {onClick: any, expanded: boolean}): JSX.Element => {
  return (
    <div className='py-2'>
      <div onClick={onClick} className={`${expanded ? 'hidden h-0' : 'block'} cursor-pointer mt-0.5 border border-gray-200 shadow-sm rounded-full flex flex-row items-center gap-x-4 py-0.5 bg-white`}>
